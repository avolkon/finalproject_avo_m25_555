...
def setup_logging() -> None:
    ...
    try:
        ...
        log_backup_count = settings.get('log_backup_count', 5)       # Количество backup файлов
        ...
        root_logger.info(f"Система логирования настроена: формат={log_format}, "
                        f"уровень={log_level_name}, директория={logs_dir}")
        
    except ConfigError as e:
        # Ошибка конфигурации - используем базовую настройку
        logging.basicConfig(level=logging.INFO)
        logging.error(f"Ошибка конфигурации логирования, используется базовый режим: {e}")
        raise
...
def _setup_specialized_loggers(logs_dir: Path, formatter: logging.Formatter, 
                              log_level: int) -> None:
    """Настройка специализированных логгеров для разных типов операций.
