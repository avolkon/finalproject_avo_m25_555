...
class RatesCache:
    ...
    def __init__(self, filepath: str = "data/rates.json") -> None:
        ...
        try:
            ...
            raise CacheError(
                f"Не удалось создать директорию: {e}", 
                operation="init"
            ) from e
    ...
    def get_rate(
        ...
        if self._cache_data is None:
            raise CacheError(
                "Не удалось загрузить данные кэша",
                operation="get_rate"
            )
    ...
    def update_rate(
        ...
        self.logger.debug(
            f"Обновление курса для {pair}: {rate} от {source} "
            f"в {update_time}"
        )
    ...
    def bulk_update(self, rates: Dict[str, Dict[str, Any]]) -> int:
        ...
        for pair, new_data in rates.items():
            try:
                ...
                self.logger.error(
                    f"Ошибка обработки пары {pair}: {e}",
                    exc_info=False
                )
                continue
    ...
    def is_fresh(
        ...
        try:
            ...
            self.logger.warning(f"Некорректный формат timestamp для пары {currency_pair}")
            return False
    ...
    def get_stale_pairs(self) -> List[str]:
        ...
        self.logger.info(
            f"Найдено {len(stale_pairs)} пар с устаревшими данными"
        )
    ...
    def _load_cache(self) -> None:
        ...
        if not self.filepath.exists():
            ...
            self.logger.info(f"Файл кэша не существует, создается новый: {self.filepath}")
        ...
        try:
            ...
            if not self._validate_cache_structure(file_data):
                ...
                self.logger.debug(
                    f"Загружено {pairs_count} пар из файла кэша"
                )
    ...
    def _validate_cache_structure(self, data: Dict[str, Any]) -> bool:
        ...
        for pair_key, pair_data in data["pairs"].items():
            if not isinstance(pair_data, dict):
                self.logger.warning(f"Некорректная структура данных для пары {pair_key}")
                return False
            ...
            for field in pair_required_fields:
                if field not in pair_data:
                    self.logger.warning(
                        f"Отсутствует поле {field} для пары {pair_key}"
                    )
                    return False
    ...
    def _atomic_write(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            raise CacheError(
                f"Ошибка атомарной записи кэша: {e}",
                operation="_atomic_write"
            ) from e
