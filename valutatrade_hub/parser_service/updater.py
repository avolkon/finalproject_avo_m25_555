...
class RatesUpdater:
    ...
    def __init__(
        ...
        self.history_storage: Optional[HistoryStorage] = history_storage  # Хранилище истории
        ...
        self.logger.info(
            f"Инициализирован RatesUpdater с {len(clients)} клиентами"
        )
    ...
    def run_update_for_source(self, source_name: str) -> UpdateResult:
        ...
        try:
            ...
            # Преобразование формата данных
            formatted_rates: Dict[str, Dict[str, Any]] = (
                self._format_rates_for_cache(rates, target_client.name)
            )
    ...
    def _save_to_history(self, source: str, rates: Dict[str, float]) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Неожиданная ошибка при сохранении истории: {e}",
                exc_info=True
            )
    ...
    def _update_cache(self, rates: Dict[str, Dict[str, Any]]) -> int:
        ...
        try:
            ...
            if "pairs" not in existing_data or not isinstance(existing_data["pairs"], dict):
                existing_data = {
                    "version": "1.0",
            ...
            for pair, new_data in rates.items():
                ...
                if existing_pair_data is None:
                    ...
                    try:
                        ...
                        existing_data["pairs"][pair] = new_data
                        updated_count += 1
                        self.logger.debug(f"Обновлена пара (некорректное время): {pair}")
