...
class HistoryStorage:
    ...
    def save_record(self, record_data: Dict[str, Any]) -> str:
        ...
        required_fields: List[str] = [
            "from_currency", "to_currency", "rate", "timestamp", "source"
        ]
    ...
    def save_batch(self, records: List[Dict[str, Any]]) -> List[str]:
        ...
        for i, record in enumerate(records):
            try:
                ...
                self.logger.debug(f"Подготовлена запись {i+1}/{len(records)}: {record_id}")
                
            except Exception as e:
                # Ошибка обработки записи - отмена всей операции
                raise StorageError(
                    f"Ошибка в записи {i}: {e}",
    ...
    def get_by_currency(
        ...
        for record in self._data["records"]:
            ...
            if (record.get("from_currency", "").upper() == search_code or 
                record.get("to_currency", "").upper() == search_code):
    ...
    def get_by_period(
        ...
        if start_dt > end_dt:
            raise ValueError(
                f"Начальная дата {start_date} позже конечной {end_date}"
            )
    ...
    def _load_data(self) -> None:
        ...
        try:
            ...
            if not self._validate_data_structure(file_data):
                self.logger.warning(
                    "Некорректная структура файла, создается новая"
                )
                self._data = self._create_default_structure()
            else:
    ...
    def _validate_data_structure(self, data: Dict[str, Any]) -> bool:
        ...
        required_top_fields: List[str] = [
            "version", "last_updated", "total_records", "records"
        ]
        ...
        try:
            ...
            if self.filepath.exists():
                try:
                    ...
                    self.logger.warning(
                        f"Не удалось создать backup: {backup_error}"
                    )
    ...
    def _verify_file_integrity(self, filepath: Path) -> None:
        ...
        try:
            ...
            raise StorageError(
                f"Ошибка проверки целостности: {e}",
                operation="_verify_file_integrity"
            ) from e
