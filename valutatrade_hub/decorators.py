...
def log_action(action: Optional[str] = None, verbose: bool = False) -> Callable:
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args: Tuple[Any, ...], **kwargs: Dict[str, Any]) -> Any:
            ...
            log_context = {"action": action_name, **context}
            ...
            if verbose:
                ...
                start_time = (
                    time.time()
                )  # Запись времени начала для измерения длительности
            ...
            try:
                ...
                logger.info(
                    f"Операция {action_name} выполнена успешно", extra=log_context
                )
                ...
                logger.error(
                    f"Ошибка операции {action_name}: {e}",
                    extra=log_context,
                    exc_info=verbose,
                )
        ...
        for param_name, param_value in bound_args.arguments.items():
            ...
            if param_name in ["user_id", "username"]:
                ...
                context["currency_code"] = (
                    param_value.upper() if isinstance(param_value, str) else param_value
                )
